...
public class Deconv2DDrawer : LayerDrawerBase<Deconvolution2D> {
    static Dictionary<int, Cache> cache = new Dictionary<int, Cache>();


    public override void DrawInspector(Layer l) {
        Deconvolution2D dl = l as Deconvolution2D;
        int numFilters = dl.outputShape[2];
        ...
        c.isExpanded = EditorGUILayout.Foldout(c.isExpanded, "<color=green>Deconvolution2D</color> - " + dl.outputShape.TOSTRING(), gs);
        if (c.isExpanded) {
            ...
            void Visualize() {
                for (int i = 0; i < numFilters; i++) {
                    ...
                    Tensor img = dl.Visualize(NeuralNetworkAssetDrawer.currentNetwork, i);
