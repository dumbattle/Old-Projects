    ...
    void DisplayChannelInfo(Rect pos) {
        ...
        foreach (var c in Graph.channel) {
            if (c.Length > 0) {
                ...
                c.autoYRange = EditorGUI.Toggle(new Rect(r.x + RANGE_POS + 100, r.y, TOGGLE_WIDTH, r.height), c.autoYRange);

                if (c.autoYRange) {
                    c.CalculateYRange();
                    EditorGUI.LabelField(new Rect(r.x + YMIN_POS, r.y, 25, r.height), "Max");
                    c.yMax = EditorGUI.FloatField(new Rect(r.x + YMIN_POS + 25, r.y, 65, r.height), c.yMax);

                    EditorGUI.LabelField(new Rect(r.x + YMIN_POS + 90, r.y, 25, r.height), "Min");
                    c.yMin = EditorGUI.FloatField(new Rect(r.x + YMIN_POS + 115, r.y, 65, r.height), c.yMin);
