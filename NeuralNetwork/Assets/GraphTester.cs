...
public abstract class AC : CustomModel {
    ...
    public Experience Sample(params (string ph, Tensor val)[] state) {
        ...
        Compute(state);
        exp.output = this["Actor"].GetTensor();
...
public abstract class Model_base {
    ...
    public ReadOnlyTensor this[string name] {
        get => forwardOps[name].value.AsReadOnly();
    }
...
public class NeuralModel : Model_base {
    ...
    public ReadOnlyTensor Compute(Tensor input) {
        Compute((INPUT_PH_NAME, input));
        return outputOp.value.AsReadOnly();
    }
